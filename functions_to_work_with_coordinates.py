...
import requests
from from_json_to_dataset import *
...
def get_coordinates_from_address_2_gis(address):
    ...
    response = requests.get(
        f'https://catalog.api.2gis.com/3.0/items/geocode?q={address}&fields=items.point&key=ruaofg3859')
    data = response.text
    data = json.loads(data)
...
def get_distance_between_two_coordinates(sources_point, list_of_targets):
    ...
    response = requests.post(
        'https://routing.api.2gis.com/get_dist_matrix?key=9ea2d981-c362-441e-b53e-d5dc26add861&version=2.0', data)
    data = response.text
    data = json.loads(data)
