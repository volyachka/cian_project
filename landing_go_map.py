...
MID_PRICE = df[['price_mortgage']].values.mean()
...
def map_from_df(data, selected):
    ...
    map = go.Scattermapbox(
        ...
        mode="markers",
        marker=dict(
            color=data["price_log"],
            showscale=False,
            cmin=log(MIN_PRICE),
            cmax=log(MAX_PRICE),
            size=2 * data["rooms_cnt"],
            colorscale=px.colors.sequential.dense,
            cauto=True
        ),
        selectedpoints=selected,
        unselected=dict(marker={"color": "#ffff00"}),
        customdata=(data["index"]),
        hoverinfo="text",
        text=data["title"] + "<br>" + "Цена: " + data["price_spaces"] + " руб."
    )
    ...
    layout = go.Layout(
        ...
        mapbox=go.layout.Mapbox(
            ...
            bearing=10,
            center=go.layout.mapbox.Center(dict(lon=37.6225, lat=55.75)),
            pitch=5,
            zoom=12,
            style="streets"
        )
...
def update_flats_list(selectedData):
...
def update_map(min_price, max_price, metro):
    ...
    return map_from_df(df, df_filtered['index'].values)
